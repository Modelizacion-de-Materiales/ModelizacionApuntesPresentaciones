...
\begin{frame}<presentation>[label=FrameEstructuraIO]
  ...
  \begin{tikzpicture}
    ...
    \texttt{linea = read(archivo)}\\
    };
    \node [text width=0.35\textwidth, fill=Khaki, 
    below=0.5cm of abrir.south west ]  (escribir) 
    {
      Escribir en forma secuencial\\
      \vspace{0.5cm}
    \texttt{fprintf(archivo, linea de texto, formato)}\\
    };
    \node [ fill=BrickRed, minimum width=0.5\textwidth, 
    below=3cm of abrir,  text=white]
    (cerrar) {CERRAR};
    ...
    \node[text width=0.45\textwidth, fill=Khaki, 
    below=0.5cm of cerrar.south west] (save) {\texttt{save('B.txt','B','-ascii')}};

    \node[text width=0.45\textwidth, fill=Khaki, 
    below=0.5cm of cerrar.south east] (save) {\texttt{save('B.txt','B','-ascii')}};
